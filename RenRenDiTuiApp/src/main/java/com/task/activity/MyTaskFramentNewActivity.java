 ...
 * 我的任务列表页--new
 */
public class MyTaskFramentNewActivity extends BaseFragmentActivity implements
        OnClickListener , BaseFragmentActivity.MyTaskInterface {
    private ViewPager vp_task_my;
    private MyFragmentPagerAdapter viewPagerAdapter;
    private List<BaseFragment> fragmentList;
    private FragmentTask_0 fragmentThroughTask;// 进行中的任务
    private FragmentTask_1 fragmentInvalidTask;// 已过期任务
...
//		menu
    ...
    private void initViewPager(int topage) {
        fragmentThroughTask = new FragmentTask_0();
        fragmentInvalidTask = new FragmentTask_1();
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        ...
        if (event.getKeyCode() == KeyEvent.KEYCODE_BACK) {
            if (!isQuit ) {
                ...
                MyTaskFramentNewActivity.this.finish();
                System.exit(0);
            }
        }
        return true;
