 ...
 * 
 */
public class MyTaskMainActivity extends BaseFragmentActivity implements
		OnClickListener {
	private Context context;
	public ViewPager vp_task_main;
	private MyFragmentPagerAdapter viewPagerAdapter;
	private List<BaseFragment> fragmentList;
	private FragmentNoGoingTask fragment_noGoingTask;// 已领取任务页
	private FragmentOnGoingTash fragment_onGoingTask;// 审核中任务页
	private FragmentFinishedTask fragment_finishedTask;// 未通过任务页
	private int topage = ToMainPage.已领取.getValue();// intent指向要显示的页面
	...
	protected void onNewIntent(Intent intent) {
		...
		if (intent != null) {
			topage = intent.getIntExtra("to", ToMainPage.已领取.getValue());
		}
	...
	public void initViewPager(int topage) {
		fragment_noGoingTask = new FragmentNoGoingTask(layoutTopMenu);
		...
		fragmentList.add(fragment_noGoingTask);
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (resultCode == RESULT_OK) {
			topage = ToMainPage.已领取.getValue();
			vp_task_main.setCurrentItem(topage);
		}
	}
