 ...
 * 资料任务审核 分组 --未通过
 */
public class FragmentMateriaTask_2 extends BaseFragment implements
	...
	private RQHandler<RSMyMaterialTaskTeamModel> rqHandler_teamTask = new RQHandler<>(
			new IRqHandlerMsg<RSMyMaterialTaskTeamModel>() {
				...
				public void onNetworknotvalide() {
					...
					FragmentMateriaTask_2.this.onNodata(
				...
				public void onSuccess(RSMyMaterialTaskTeamModel t) {
					...
					if (pageindex == 1) {
						if (t.getData().getCount() == 0) {
							pulltorefresh_taskList.setVisibility(View.GONE);
							FragmentMateriaTask_2.this.onNodata(
									ResultMsgType.Success, 0,R.string.material_over_no_data, "",
									FragmentMateriaTask_2.this);
						} else {
				...
				public void onSericeErr(RSMyMaterialTaskTeamModel t) {
					pulltorefresh_taskList.setVisibility(View.GONE);
					FragmentMateriaTask_2.this.onNodata(
							ResultMsgType.ServiceErr, 0,R.string.every_no_data_error,"", FragmentMateriaTask_2.this);
				}
				...
				public void onSericeExp() {
					pulltorefresh_taskList.setVisibility(View.GONE);
					FragmentMateriaTask_2.this.onNodata(
							ResultMsgType.ServiceExp, 0,R.string.every_no_data_error, "", FragmentMateriaTask_2.this);
	...
	public void getInitData() {
		...
		ApiUtil.Request(new RQBaseModel<RQMyMaterialTaskTeamModel, RSMyMaterialTaskTeamModel>(
				context, new RQMyMaterialTaskTeamModel(Utils.getUserDTO(context).data.userId,"0","10","3"),
...
//	}
	...
	public void getMoreData() {
        ApiUtil.Request(new RQBaseModel<RQMyMaterialTaskTeamModel, RSMyMaterialTaskTeamModel>(
                context, new RQMyMaterialTaskTeamModel(Utils.getUserDTO(context).data.userId,nextId,"10","3"),
                new RSMyMaterialTaskTeamModel(), ApiNames.资料审核详情分组后列表.getValue(),
                RequestType.POST, rqHandler_teamTask));
		pageindex++;
	}
