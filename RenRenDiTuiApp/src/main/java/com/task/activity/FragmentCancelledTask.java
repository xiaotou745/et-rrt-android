...
import com.task.service.GetCancelledTaskAdapter;
 ...
 * 
 */
public class FragmentCancelledTask extends BaseFragment implements
		OnHeaderRefreshListener, OnFooterRefreshListener, INodata {
	private View view;
	private Context context;
	private ListView lv_task_main;
	private GetCancelledTaskAdapter getCancelledTaskAdapter;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		getCancelledTaskAdapter = new GetCancelledTaskAdapter(context, finishedTaskInfos);
		lv_task_main.setAdapter(getCancelledTaskAdapter);
		return view;
	}
	...
	public void getInitData() {
		ApiUtil.Request(new RQBaseModel<RQGetFinishedTask, RSGetFinishedTask>(
				context, new RQGetFinishedTask(Utils.getUserDTO(context).data.userId, "0",2),
	...
	public void getMoreData() {
		ApiUtil.Request(new RQBaseModel<RQGetFinishedTask, RSGetFinishedTask>(
				context, new RQGetFinishedTask(Utils.getUserDTO(context).data.userId, nextId,2),
				new RSGetFinishedTask(), ApiNames.获取所有已提交的任务.getValue(),
				RequestType.POST, rqHandler_getOnGoingTask));
		pageindex++;
	}
