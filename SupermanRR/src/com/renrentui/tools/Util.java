...
public class Util {
    ...
    public static String getLargePicture(String imgUrl) {
        try {
            ...
            return stringBuffer.append(string.substring(0, string.length() - 4)).append("_ol.")
                    .append(string.substring(string.length() - 3, string.length())).toString();
        } catch (Exception e) {
            return null;
        }
    }
    ...
    public static String getSmallPicture(String imgUrl) {
        try {
            StringBuffer stringBuffer = new StringBuffer();
            String string = imgUrl.trim();
            return stringBuffer.append(string.substring(0, string.length() - 4)).append("_os.")
                    .append(string.substring(string.length() - 3, string.length())).toString();
        } catch (Exception e) {
            return null;
        }
    ...
    public static String getDayTimeString(int cutdays) {
        ...
        return time.append(year).append("-").append(monthString).append("-").append(dayString).toString();
    ...
    public static String getString(Context context, int resid1, int resid2) {
        return context.getResources().getString(resid1) + context.getResources().getString(resid2);
    }

    public static String getString(Context context, int resid1, String str, int resid2) {
        return context.getResources().getString(resid1) + str + context.getResources().getString(resid2);
    }
    ...
    public static String getImgSmall(String url) {
        if (url != null) {
            if (!url.equals("")) {
                url = url.substring(0, url.length() - 4) + "_os" + url.substring(url.length() - 4);
            }
        }
        return url;
    }
    ...
    public static String getImgBig(String url) {
        if (url != null) {
            if (!url.equals("")) {
                url = url.substring(0, url.length() - 4) + "_ol" + url.substring(url.length() - 4);
            }
        }
        return url;
    }
    ...
    public static boolean isFirstEntry(Context context) {
        String isFirstEntry = SharedPreferencesTools.getSPInstance(context).getSharedPreferences().get("isFirstEntry");
        if (isFirstEntry == null) {
            Map<String, String> param = new HashMap<String, String>();
            param.put("isFirstEntry", "entried");
            SharedPreferencesTools.getSPInstance(context).setSharedPreferences(param);
            return true;
        } else {
            return false;
        }
    ...
    public static String TrimLeftStr(String str, String trim) {
        if (str != null && trim != null && str.length() > trim.length() && str.substring(0, trim.length()).equals(trim)) {
