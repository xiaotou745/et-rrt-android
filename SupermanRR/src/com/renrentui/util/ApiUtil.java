...
public class ApiUtil {
    ...
    private static Map<String, Boolean> apiThreadState = new HashMap<String, Boolean>();
                    ...
                    public void run() {
                        ...
                        if (NetworkValidator.isNetworkConnected(rqModel.context)) {
                            if (rqModel.RequestType == RequestType.GET) {
                                resultJson = httpRequest.get(rqModel.context, rqModel.rqModel, rqModel.getApiAddress(),
                                        0, 0);
                            } else if (rqModel.RequestType == RequestType.POST) {
                                System.out.println("rqModel ============ " + rqModel.toString());
                                resultJson = httpRequest.post(rqModel.context, rqModel.rqModel,
                            ...
                            if (resultJson == null || resultJson.trim() == "") {
                                ...
                                RS rs = (RS) GsonTools.jsonToBean(resultJson, rqModel.rsModel.getClass());
                                RSBase rsBase = (RSBase) rs;
                                if (rs != null && rsBase.code.equals("200")) {
                                    // 成功从服务器获取数据
                                    rqModel.rqHandler.sendMessage(rqModel.rqHandler.obtainMessage(
                                            ResultMsgType.Success, rs));
                                } else {
                                    // 服务器错误 获取数据失败
                                    rqModel.rqHandler.sendMessage(rqModel.rqHandler.obtainMessage(
                                            ResultMsgType.ServiceErr, rs));
                                }
                            }
                        } else {
